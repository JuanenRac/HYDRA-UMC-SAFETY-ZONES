...
def parse_calibration(data: dict) -> ZoneCalibration:
    ...
    # the old `int(data["max_age_days"])` accepted values that were
    ...
    # real authoring mistake reach `ZoneCalibration.__post_init__`'s own
    # `> 0` check looking like a genuinely valid value.
    if isinstance(raw_max_age_days, bool) or not isinstance(raw_max_age_days, int):
        raise CalibrationError(
            f"max_age_days must be a strict integer, not a boolean or fraction: {raw_max_age_days!r}"
        )
