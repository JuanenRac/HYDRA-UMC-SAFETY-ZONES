...
- **SAFE-01 (P2):** `parse_calibration()`'s old `int(data["max_age_days"])`
  ...
  raising. A `max_age_days: true` or `max_age_days: 1.5` calibration
  file was silently accepted as `max_age_days=1`. Fixed with an explicit
  `isinstance` check rejecting both cases before `ZoneCalibration`'s own
  `> 0` check ever sees them; zero and negative values were already
  rejected there. 6 new regression tests (`True`/`False`/`1.5`/`0`/`-5`,
