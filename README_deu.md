...
flowchart TB
    ...
    CAL -- Ja --> OBS{"Beobachter aktiv & frisch? - real v0"}
    OBS -- Nein --> INHIBIT
...
* **`--observation`/`"observation"` ist auf Schnittstellenebene optional, nicht auf Sicherheitsebene** - ein Aufrufer, der es nie übergibt, erhält jedes Mal ein ehrliches `INHIBITED`, nie einen Absturz und nie ein stillschweigendes `READY`; das hält die Parsing-Schicht rückwärtskompatibel für noch nicht aktualisierte Aufrufer und schließt gleichzeitig die echte Lücke, in der eine leere `objects`-Liste nicht von einem echten, aktiven Beobachter zu unterscheiden war, der die Zelle als frei bestätigt.
...
│   ├── observation.py    # Echte Beobachter-Gesundheitsverfolgung - spiegelt calibration.py
...
// einem aktiven, kürzlich aktualisierten Beobachter stammen
