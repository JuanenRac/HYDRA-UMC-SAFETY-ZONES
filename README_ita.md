...
* 👁️ **Applicazione della salute dell'osservatore (v0):** `evaluate_safety()` accetta anche uno stato `observation` opzionale (attivo/inattivo, ultimo orario di osservazione, proprio errore) - vedi [I32](docs/CLI_REFERENCE.md). Nessuna evidenza di osservazione, un osservatore disabilitato, un errore segnalato dall'osservatore, o un'osservazione obsoleta si risolvono tutti in `INHIBITED` prima che qualsiasi logica di violazione venga eseguita - esattamente come una calibrazione mancante: un rilevatore crashato o un avvio fresco non devono mai essere letti silenziosamente come "confermato libero" solo perché non sono stati segnalati oggetti.
...
**Verifica di onestà - cosa funziona davvero oggi:** l'entry point reale (`src/hydra_umc_safety_zones/main.py`) continua a stampare identità/versione/ruolo con una chiamata senza argomenti, ma ora ha anche un vero sottocomando `check --zones PERCORSO --detections PERCORSO [--observation PERCORSO]`: carica un set di zone (zone + metadati di calibrazione opzionali), posizioni di oggetti rilevati e uno stato di osservazione opzionale da JSON, controlla prima la freschezza della calibrazione, poi la salute dell'osservatore (I32 - nessuno stato di osservazione, un osservatore disabilitato/in errore/obsoleto si risolvono tutti in `INHIBITED` esattamente come una calibrazione mancante), poi esegue un vero controllo delle violazioni, richiede E-STOP per ogni violazione Danger, ed esce con 0 (Ready) / 1 (Warning) / 2 (Danger, E-STOP richiesto) / 3 (Inhibited - calibrazione assente/scaduta, o osservatore assente/disabilitato/obsoleto/in errore) a seconda del risultato. Ciò che davvero non è ancora reale: la segmentazione spaziale Hailo-8 che produrrebbe quelle posizioni di oggetti rilevati su hardware reale, il mascheramento di auto-occlusione, il vero processo osservatore la cui vivacità `--observation` segnala, e qualsiasi vero trasporto CAN per la richiesta E-STOP. Vedi [`CHANGELOG.md`](CHANGELOG.md) per ciò che è stato consegnato esattamente finora, e "Stato Attuale e Prossimi Passi" più sotto per ciò che resta aperto.

---

## 2. 🔄 FLUSSO DI LOGICA DI SICUREZZA PREVISTO

Il diagramma sotto è il flusso dati obiettivo verso cui viene costruito questo progetto. `CAL` (controllo calibrazione), `OBS` (controllo salute osservatore, I32), `ZONE` (Controllo Zona) e la ripartizione Warning/Danger successiva sono reali oggi, guidati da `evaluate_safety()` (che avvolge `check_breaches()`/`request_estop_for()`), date posizioni di oggetti rilevati da un file JSON. Tutto ciò che precede `CAL`/`OBS`/`ZONE` (la vera pipeline Hailo-8, e il vero processo osservatore la cui vivacità `OBS` controlla) e segue `STOP` (il vero trasporto CAN) resta lavoro futuro.
...
flowchart TB
    ...
    CAL -- Sì --> OBS{"Osservatore Attivo e Recente? - reale v0 (I32)"}
    OBS -- No --> INHIBIT
    OBS -- Sì --> ZONE{"Controllo Zona - reale v0"}
...
* **`--observation`/`"observation"` è opzionale a livello di interfaccia, non a livello di sicurezza (I32)** - un chiamante che non lo passa mai ottiene ogni volta un `INHIBITED` onesto, mai un crash e mai un `READY` silenzioso; questo mantiene il livello di parsing retrocompatibile per i chiamanti non ancora aggiornati, chiudendo al contempo il vero divario in cui una lista `objects` vuota era indistinguibile da un vero osservatore attivo che conferma la cella libera.
...
│   ├── observation.py    # Vero tracciamento della salute dell'osservatore (I32) - rispecchia calibration.py
...
│   ├── config.py         # Vero caricamento JSON per zone/rilevamenti/osservazione
...
├── tests/                # Test veri: geometria, violazioni, calibrazione, osservazione, safety_state, estop, config, api, CLI
...
5. **Vera suite di test** - `pytest tests/` esegue tutti i 104 test.
...
Il vero sottocomando `check` necessita di un file di zone e uno di rilevamenti, entrambi JSON semplice, più un file di stato osservazione opzionale. `calibration` è opzionale nel file di zone, e `--observation` è opzionale sulla riga di comando - vedi sotto cosa succede senza:
...
```json
// observation.json - vera evidenza che i rilevamenti sopra provengono
// davvero da un osservatore attivo e aggiornato di recente (I32)
...
Esce con `2` (Danger, E-STOP richiesto), `1` (violazione solo Warning), `0` (nessuna violazione, calibrazione valida, osservatore attivo e recente), o `3` (**Inhibited** - calibrazione assente/scaduta, o osservatore assente/disabilitato/obsoleto/in errore, controllata prima di qualsiasi logica di violazione). Esempio reale del percorso di sicurezza intrinseca - le stesse `detections.json` sopra, ma `zones.json` senza alcuna chiave `"calibration"`:
...
Esce con `3` - nota che non c'è alcun output `BREACH`/`E-STOP`, anche se `op1` si trova dentro entrambe le zone: un set di zone non affidabile non raggiunge mai il passaggio di controllo delle violazioni. Omettere del tutto `--observation` fallisce in modo altrettanto sicuro, anche con calibrazione fresca e nessun oggetto vicino a una zona:
...
La stessa logica di `check` è raggiungibile anche via HTTP, per chi non usa la CLI - `zones`/`detections`/`observation` viaggiano nel corpo JSON invece che come percorso di file:
