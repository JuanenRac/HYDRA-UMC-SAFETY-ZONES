...
# positive integers.
@pytest.mark.parametrize("bad_max_age", [True, False, 1.5, 0, -5])
def test_parse_calibration_rejects_bool_fraction_zero_and_negative_max_age(bad_max_age):
    with pytest.raises(CalibrationError):
        parse_calibration(
            {
                "version": "cal-1",
                ...
                "max_age_days": bad_max_age,
            }
        )


def test_parse_calibration_still_accepts_a_real_valid_max_age():
    # A real, valid calibration with a real reference date must keep
    # working - the fix above must not become over-strict.
    cal = parse_calibration(
        {"version": "cal-1", "source": "manual", "calibrated_at": "2026-08-01", "max_age_days": 30}
    )
    assert cal.max_age_days == 30
    assert not is_calibration_expired(cal, date(2026, 8, 15))
    assert is_calibration_expired(cal, date(2026, 12, 1))
