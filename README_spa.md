...
* 👁️ **Aplicación de salud del observador (v0):** `evaluate_safety()` también acepta un estado `observation` opcional (activo/inactivo, última hora de observación, error propio) - ver [I32](docs/CLI_REFERENCE.md). Ninguna evidencia de observación en absoluto, un observador desactivado, un error informado por el observador, o una observación caducada, todos resuelven a `INHIBITED` antes de que se ejecute cualquier lógica de brechas - exactamente igual que una calibración ausente: un detector caído o un arranque reciente nunca deben leerse silenciosamente como "confirmado despejado" solo porque no se informó ningún objeto.
...
* 🌐 **API JSON/HTTP (v0.0.7):** el subcomando `serve` expone la misma lógica exacta de `check` (`evaluate_safety()`/`check_breaches()`/`request_estop_for()`) sobre un `http.server` de la stdlib (`POST /check`, `GET /stats`) para clientes que no son la CLI - solo loopback por defecto, igual que la unidad `systemd/hydra-umc-safety-zones.service`. Ver [`docs/CLI_REFERENCE.md`](docs/CLI_REFERENCE.md) para cada comando, flag y código de salida real.
...
**Comprobación de honestidad - qué funciona hoy de verdad:** el entry point real (`src/hydra_umc_safety_zones/main.py`) sigue imprimiendo identidad/versión/rol en una llamada sin argumentos, pero ahora también tiene un subcomando real `check --zones RUTA --detections RUTA [--observation RUTA]`: carga un conjunto de zonas (zonas + metadatos de calibración opcionales), posiciones de objetos detectados y un estado de observación opcional desde JSON, comprueba primero la frescura de la calibración, luego la salud del observador (I32 - ningún estado de observación en absoluto, un observador desactivado/con error/caducado resuelven todos a `INHIBITED` exactamente igual que una calibración ausente), luego ejecuta comprobación real de brechas, solicita E-STOPs para cada brecha Danger, y sale con 0 (Ready) / 1 (Warning) / 2 (Danger, E-STOP solicitado) / 3 (Inhibited - calibración ausente/caducada, u observador ausente/desactivado/caducado/con error) según el resultado. Lo que de verdad todavía no es real: la segmentación espacial de Hailo-8 que produciría esas posiciones de objetos detectados en hardware real, el enmascarado de auto-oclusión, el proceso observador real cuya vitalidad informa `--observation`, y cualquier transporte CAN real para la solicitud de E-STOP. Ver [`CHANGELOG.md`](CHANGELOG.md) para lo entregado exactamente hasta ahora, y "Estado Actual y Próximos Pasos" más abajo para lo que sigue abierto.

---

## 2. 🔄 FLUJO DE LÓGICA DE SEGURIDAD PREVISTO

El diagrama de abajo es el flujo de datos objetivo hacia el que se construye este proyecto. `CAL` (comprobación de calibración), `OBS` (comprobación de salud del observador, I32), `ZONE` (Comprobación de Zona) y el reparto Warning/Danger que sigue son reales hoy, impulsados por `evaluate_safety()` (que envuelve a `check_breaches()`/`request_estop_for()`), dadas posiciones de objetos detectados desde un fichero JSON. Todo lo anterior a `CAL`/`OBS`/`ZONE` (el pipeline real de Hailo-8, y el proceso observador real cuya vitalidad comprueba `OBS`) y posterior a `STOP` (el transporte CAN real) sigue siendo trabajo futuro.
...
flowchart TB
    ...
    CAL -- Sí --> OBS{"¿Observador Activo y Fresco? - real v0 (I32)"}
    OBS -- No --> INHIBIT
    OBS -- Sí --> ZONE{"Comprobación de Zona - real v0"}
...
* **`--observation`/`"observation"` es opcional a nivel de interfaz, no a nivel de seguridad (I32)** - un llamante que nunca lo pasa obtiene un `INHIBITED` honesto cada vez, nunca un fallo y nunca un `READY` silencioso; esto mantiene la capa de parsing retrocompatible para llamantes aún no actualizados, mientras cierra la brecha real donde una lista `objects` vacía era indistinguible de un observador real y activo confirmando la celda despejada.
...
│   ├── observation.py    # Seguimiento real de salud del observador (I32) - refleja calibration.py
...
│   ├── config.py         # Carga real de JSON para zonas/detecciones/observación
...
├── tests/                # Tests reales: geometría, brechas, calibración, observación, safety_state, estop, config, api, CLI
...
5. **Suite de tests real** - `pytest tests/` ejecuta los 104 tests.
...
El subcomando real `check` necesita un fichero de zonas y uno de detecciones, ambos JSON plano, más un fichero de estado de observación opcional. `calibration` es opcional en el fichero de zonas, y `--observation` es opcional en la línea de comandos - ver abajo qué pasa sin ellos:
...
```json
// observation.json - evidencia real de que las detecciones de arriba
// realmente vinieron de un observador activo y actualizado recientemente (I32)
...
Sale con `2` (Danger, E-STOP solicitado), `1` (brecha solo Warning), `0` (sin brecha, calibración válida, observador activo y fresco), o `3` (**Inhibited** - calibración ausente/caducada, u observador ausente/desactivado/caducado/con error, comprobado antes de cualquier lógica de brechas). Ejemplo real del camino de fallo seguro - las mismas `detections.json` de arriba, pero `zones.json` sin ninguna clave `"calibration"`:
...
Sale con `3` - fíjate en que no hay salida `BREACH`/`E-STOP` en absoluto, aunque `op1` esté dentro de ambas zonas: un conjunto de zonas no fiable nunca llega al paso de comprobación de brechas. Omitir `--observation` por completo falla igual de seguro, incluso con calibración fresca y ningún objeto cerca de una zona:
...
La misma lógica de `check` también es accesible por HTTP, para clientes que no son la CLI - `zones`/`detections`/`observation` viajan en el cuerpo JSON en vez de una ruta de fichero:
...
* **`check` sale con código distinto de cero** - eso es comportamiento real y correcto, no un fallo: `1` significa que se encontró una brecha solo Warning, `2` significa que una brecha Danger solicitó un E-STOP, `3` significa que la calibración del conjunto de zonas está ausente/caducada o el estado del observador está ausente/desactivado/caducado/con error (fallo seguro, comprobado antes de que se ejecute cualquier lógica de brechas). Solo un traceback de Python o un error de JSON malformado es un fallo de verdad.

---

## 🚀 Estado Actual y Próximos Pasos

**Qué funciona hoy:** definiciones reales de zonas Warning/Danger y comprobación de brechas (`geometry.py`/`zones.py`/`breach.py`), aplicación real de frescura de calibración que falla de forma segura a `INHIBITED` antes de ejecutar cualquier lógica de brechas (`calibration.py`/`safety_state.py`), un pipeline real de *solicitud* de E-STOP que respeta el límite detectar-vs-aplicar por construcción (`estop.py`), un subcomando CLI real `check` sobre ficheros JSON de zonas/detecciones, y 104 tests pasando - ver [`CHANGELOG.md`](CHANGELOG.md) para la salida completa real de build/run.
